<#
.SYNOPSIS
    Open Codex worktree in a new VS Code window

.DESCRIPTION
    Opens the Codex worktree (.worktrees/codex) in a new VS Code window.
...
.EXAMPLE
    .\scripts\open_codex_workspace.ps1
    Opens the Codex worktree in a new window

.EXAMPLE
    .\scripts\open_codex_workspace.ps1 -Setup
...
$WorkspaceFile = ".worktrees/Codex.code-workspace"
...
if (-not (Test-Path $FullWorktreePath)) {
    if ($Setup) {
        ...
        & "$RepoRoot\scripts\setup_codex_worktree.ps1"
        if ($LASTEXITCODE -ne 0) {
            Write-Error "Failed to create worktree"
            exit 1
        }
    } else {
        Write-Error "Worktree does not exist: $FullWorktreePath`nRun with -Setup to create it, or run: .\scripts\setup_codex_worktree.ps1"
        exit 1
    }
}
...
Write-Host "Opening Codex workspace in new VS Code window..." -ForegroundColor Cyan
