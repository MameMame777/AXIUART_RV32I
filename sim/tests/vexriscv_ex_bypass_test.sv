...
//   - No stall detected in trace (proves bypass works)
//==============================================================================
...
class vexriscv_ex_bypass_test extends vexriscv_base_test;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        // Verify register values + stall-free execution
        test_passed = (x1_val == 5) && (x2_val == 6) && (!stall_found);

        if (test_passed) begin
            `uvm_info(get_type_name(),
                $sformatf("PASS: x1=%0d, x2=%0d, stall_delta=%0d (EX bypass working)",
                          x1_val, x2_val, perf_stall_delta),
                UVM_NONE)
        end else begin
            `uvm_error(get_type_name(),
                $sformatf("FAIL: x1=%0d(exp:5), x2=%0d(exp:6), stall=%0d, stall_delta=%0d",
                          x1_val, x2_val, stall_found, perf_stall_delta))
        end
